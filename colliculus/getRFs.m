...
saveName = 'pre_sparse_noise_RFs.mat';
...
for k = 1:length(db)
    for t = 1:length(db(k).tags), thisTag = db(k).tags{t};
        ...
        if ~overwrite && exist([saveFolder saveName],'file')
            continue
        end
        
        %         spks = loadNeuralData(ksDir,dataDir); % loads all neural data
        spks = loadKSdir(ksDir);
        foo = dir([dataDir '\*.lf.bin']);
        spks.lfp_path = foo.name;
        spks.nSampLFP = round(foo.bytes/2/spks.n_channels_dat);
        spks.channel_map = readNPY([ksDir 'channel_map.npy']);
        spks.channel_map = double(spks.channel_map);
        
        if isfield(db,'tlExp') % typical case
        ...
        else % case where we use the pre-experiment RF mapping
            thisBlock = dir([blockDir '\' num2str(db(k).noiseExp) '\*_Block.mat']);
            blk = load([thisBlock.folder '\' thisBlock.name]);
            [st,sp] = eventTimesLocal(dataDir,blk.block);
            
            stimTimes_local = st;
            stimPosition = sp;
        end
        
        %% correct existing (if requested)
        if exist([saveFolder saveName],'file') && exist('paragon','var') 
            if verbose % save time by only fixing what is missing
                disp(['Fixing: ' db(k).mouse_name ' on ' db(k).date ', ephys_' thisTag]); 
            end
            g = load(paragon);
            snrf = loadVar([saveFolder saveName],'snrf');
        ...
        for iCell = 1:nCells
            theseSpikes = spks.st(spks.clu == spks.cids(iCell));
